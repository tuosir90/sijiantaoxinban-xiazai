/**
 * 品牌定位分析页面逻辑（FastAPI后端：生成Markdown + PDF）。
 */
...
function collectPayload() {
  ...
  const mainProducts = ($("mainProducts").value || "").trim();

  const targetGroups = Array.from(document.querySelectorAll('input[name="targetGroup"]:checked'))
    .map(el => (el.value || "").trim())
    .filter(Boolean);

  const price = parseInt($("priceRange").value || "0", 10) || 0;
...
async function onGenerate() {
  ...
  try {
    ...
    const data = await generateReport({ module: "brand", payload, screenshotFile: null });

    $("markdown").value = data.markdown || "";
    $("preview").src = toAbsoluteUrl(data.preview_url);
    $("btn-download").dataset.pdfUrl = data.pdf_url || "";
    $("btn-download").dataset.fallbackName = `${payload.storeName}_品牌定位分析.pdf`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("btn-reset").addEventListener("click", () => {
    ...
    $("address").value = "";
    $("mainProducts").value = "";
    document.querySelectorAll('input[name="targetGroup"]').forEach(el => (el.checked = false));
    $("priceRange").value = "60";
    $("priceValue").textContent = "60";
    resetPage();
  });
