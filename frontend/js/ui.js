...
async function onGenerate() {
  ...
  try {
    ...
    const screenshotFile = getScreenshotFileIfNeeded();

    const data = await generateReport({ module, payload, screenshotFile });

    $("markdown").value = data.markdown || "";
    $("preview").src = toAbsoluteUrl(data.preview_url);
    $("btn-download").dataset.pdfUrl = data.pdf_url || "";
    $("btn-download").dataset.fallbackName = `${data.report_id || "report"}.pdf`;
    $("btn-download").disabled = !data.pdf_url;

    setText("status", `已生成（报告ID：${data.report_id}，有效期至：${(data.expires_at || "").replace("T", " ").slice(0, 19)}）`);
  } catch (e) {
    setText("status", "");
